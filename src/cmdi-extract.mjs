...
const DIRECT_PATTERNS = [
  ...
  // `exec(\`ping ${req.query.host}\`)` never fired. `(?<!\.)` keeps `myRegex.exec(...)` out ; the Node-only source
  // (`req.query|body|params`) keeps Python's `exec(request.args…)` on the py-eval rule ; `execFile`/`spawn` take argv
...
const HAS_SINK = /\bos\.(?:system|popen)|subprocess\.|(?<!\.)\b(?:eval|exec|execSync)\s*\(|(?:child_process|cp)\s*\.\s*(?:exec|execSync)|new\s+Function/;
...
const sink = (v) => {
  ...
  return new RegExp(
    ...
    `|(?<!\\.)\\b(?:eval|exec|execSync)\\s*\\([^)]*\\b${V}\\b` +   // bare exec/execSync = destructured child_process (B-P1-6)
...
/** The cmdi wrapper config — shared by the walker's cross-file registry build and the calibration harness. */
